...
async function silverbot () {
    ...
    if (Session_Db.length < 1) {
        ...
        conn.loadAuthInfo(Session.deCrypt(Session_Db[0].dataValues.value));
    }
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('[⚓]─⦁ 𝚂𝙸𝙻𝚅𝙴𝚁')}${chalk.blue.bold(' 𝙱𝙾𝚃 ⦁─[⚓]')}
${chalk.white.bold('Version:')} ${chalk.red.bold(config.VERSION)}
${chalk.blue.italic('ℹ️ Silver bot WhatsApp වෙත සම්බන්ධ වෙමින් ... කරුණාකර රැඳී සිටින්න.')}`);
    });
    ...
    conn.on('open', async () => {
        ...
        if (config.WORKTYPE == 'private') { 
         if (config.LANG == 'EN') { 
             ...
             var privateen = '*[⚓]─⦁ 𝚂𝙸𝙻𝚅𝙴𝚁 𝙱𝙾𝚃 ⦁─[⚓] Working as Private! 📍*\n\n```Please do not use commands here. This is your LOG number. 📈```\n\n```You can use commands in any other chat 😜```\n\n*Your bot is working as private.*\n*To change it, use* \n.setvar WORK_TYPE:public\n\n\n```Thanks For Using ⎝⚓⎠ silverbot ⎝⚓⎠```'
             await conn.sendMessage(conn.user.jid, Buffer.from(userjidlogo.data), MessageType.image, {mimetype: Mimetype.jpg, caption: privateen});
             
         } else if (config.LANG == 'SI') {
             
             var privatesi = '*[⚓]─⦁ 𝚂𝙸𝙻𝚅𝙴𝚁 𝙱𝙾𝚃 ⦁─[⚓] ප්‍රයිවට් ආකාරයට ක්‍රියා කරයි 📍*\n\n```කරුණාකර මෙහි plugins භාවිත නොකරන්න.මෙය ඔබගේ log අංකයයි. 📈```\n\n```වෙනත් ඕනෑම chat එකක විධානයන් භාවිත කළ හැකිය. 😜```\n\n*ඔබේ bot private ආකරයෙන් ක්‍රියාකරයි.*\n*එය වෙනස් කිරීමට* ,\n.setvar WORKTYPE:public\n*යන විධානය භාවිත කරන්න.*\n\n\n```⎝⚓⎠ silverbot ⎝⚓⎠ භාවිත කරනවාට ස්තූතියි```'
             await conn.sendMessage(conn.user.jid,Buffer.from(userjidlogo.data), MessageType.image, {mimetype: Mimetype.png, caption: privatesi})
        }
              }else if (config.WORKTYPE == 'public') { 
                  
         if (config.LANG == 'EN') { 
            var publicen = '*[⚓]─⦁ 𝚂𝙸𝙻𝚅𝙴𝚁 𝙱𝙾𝚃 ⦁─[⚓] Working as Public! 🌐*\n\n```Please do not try plugins here. This is your LOG number. 📈```\n\n```You can use commands in any other chat 😜```\n\n*Your bot is working as public.*\n*To change it, use* \n.setvar WORK_TYPE:private\n\n\n```Thanks For Using ⎝⚓⎠ silverbot ⎝⚓⎠```'
              await conn.sendMessage(conn.user.jid,Buffer.from(userjidlogo.data), MessageType.image, {mimetype: Mimetype.png, caption: publicen})

         } else if (config.LANG == 'SI') {
             
             var publicsi = '*[⚓]─⦁ 𝚂𝙸𝙻𝚅𝙴𝚁 𝙱𝙾𝚃 ⦁─[⚓] පබ්ලික් ආකාරයට ක්‍රියා කරයි 🌐*\n\n```කරුණාකර මෙහි plugins භාවිත නොකරන්න.මෙය ඔබගේ log අංකයයි. 📈```\n\n```වෙනත් ඕනෑම chat එකක විධානයන් භාවිත කළ හැකිය. 😜```\n\n*ඔබේ bot public ආකරයෙන් ක්‍රියාකරයි.*\n*එය වෙනස් කිරීමට* ,\n.setvar WORKTYPE:private\n*යන විධානය භාවිත කරන්න.*\n\n\n```⎝⚓⎠ silverbot ⎝⚓⎠ භාවිත කරනවාට ස්තූතියි```'
              await conn.sendMessage(conn.user.jid,Buffer.from(userjidlogo.data), MessageType.image, {mimetype: Mimetype.png, caption: publicsi})
        }
       }
     });
  ...
  var wellogo = await axios.get(config.LOGO_WELCOME, { responseType: 'arraybuffer' })
   if (gb !== false) {
      await conn.sendMessage(msg.key.remoteJid, Buffer.from(wellogo.data), MessageType.video, {mimetype: Mimetype.gif , caption:  gb.message})
      }}else {
      var wellogo = await axios.get(config.LOGO_WELCOME, { responseType: 'arraybuffer' })
   if (gb !== false) {
      await conn.sendMessage(msg.key.remoteJid, Buffer.from(wellogo.data), MessageType.image, {mimetype: Mimetype.png , caption:  gb.message})
      }
      }
      return; 
      }
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, '*⚒ ⦁ දෝෂ වාර්තාව (🪐 SILVER BOT 🪐) ⦁ ⚒ ︎*' + 
                                    ...
                                    '\n_ඔබට දෝෂය මෙම කණ්ඩායමට යොමු කළ හැකිය  ._\n\n' +
                                    '*සිදුවන දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false});

                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚒ ⦁ දෝෂ විසඳුම (🪐 SILVER BOT 🪐) ⦁ ⚒ ︎*' + 
                                        '\n========== ```දෝෂය කියවන්න!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [🪐 SILVER BOT 🪐] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '* 💱️ ERROR ANALYSIS [🪐 SILVER BOT 🪐]  💱️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
